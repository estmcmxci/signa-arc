...
**ENS Labs' own framing, 2026-09-04** — *"ENS as a Registry Layer for Tokenized Assets"* (ens.domains/blog/post/ens-registry-tokenized-assets, author enslabs.eth). It reads like a specification for this workstream. The bites, with what each licenses:

| ENS Labs, verbatim | What it means for us |
|---|---|
| *"the registry layer that connects an asset's identity to the contracts, records, and systems built around it"* | A **facility** is the asset. Its name resolves to the vault, the engine, the policy, and the approved issuers. |
| *"issuers retain control over how their assets are organized and can decide who is authorized to manage and update the information associated with them"* | That is EAC. The lender decides who may attest; the right to write a record is a revocable role. |
| *"institutional assets… often involve issuers, administrators, custodians, and other service providers. ENS can reflect those existing relationships while giving each party the appropriate level of control."* | Our roles exactly: lender admin, servicer as exposure issuer, verifier or broker as hedge issuer — each with only the rights it needs on the facility's namespace. |
| *"Issuers and platforms publish information within their namespaces, and applications choose which namespaces they trust and use."* | The approved-issuer set **is** a choice of trusted namespaces. Revocation is un-trusting one. |
| *"Its hierarchical registry architecture allows each name to have its own registry for the names below it, enabling any asset issuer to create a taxonomy"* | Lender → facilities → approved verifiers, as a hierarchy of registries. This is the ENSv2 feature the track rewards, used for its stated purpose. |
| *"…records could identify the issuer, ticker, asset class, underlying asset, supported networks, token standard, eligible investor type, key service providers, and official documentation"* | A facility profile's records: currency pair, policy hash, chain, vault and engine addresses, exposure issuer, hedge issuers, and the FX provider as a **key service provider**. |
| *"Custody, settlement, and transfer restrictions would remain with the relevant providers."* | The same boundary discipline as our exclusions. ENS names; it does not custody. Signa gates; it does not custody or execute. |

**Design upgrade for Workstream D.** The verifier namespace alone under-uses this. The stronger build is a **named facility profile** — `<facility>.<lender>.eth` on ENSv2 Sepolia — whose records resolve to the facility's contracts and policy, with **verifier subnames beneath it** carrying EAC record-write roles. The coverage contract resolves the facility name → approved verifier set → credential. Revoke a verifier's role and the credential stops resolving and the drawdown pauses. That is ENS Labs' asset-profile model applied to a credit facility, and it uses the hierarchical registry for the taxonomy they describe rather than as a gimmick.

Two more things from the post worth having in hand: ENS Labs name **Base, Coinbase and Chainlink in one sentence** — *"On Base, Coinbase and Chainlink are connecting tokenized stocks to pricing infrastructure that can support lending, borrowing, and other DeFi use cases"* — which is our stack described by a third sponsor; and they cite **tokenised Treasuries at $15B, up 15x in two years**, the market the facility profile sits inside.
