...
// Copied from apps/dashboard/src/manifest.ts at 5ed7f9a: the types, `validateManifest` and its
// helpers, unchanged. Left behind on purpose: the Vite `import.meta.glob` loader and the bundled
// fixture fallback. The CLI must never substitute a fixture for a missing manifest (ERD §4).
// The dashboard keeps its own copy until its workstream switches its import to this package.
